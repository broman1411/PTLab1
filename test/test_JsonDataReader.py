...
class TestJsonDataReader:
    ...
    def test_read_json_with_list_structure(self, tmpdir):
        """Тест обработки JSON с неправильной структурой (список)"""
        invalid_data = [
            {
                "name": "Студент",
        ...
        with pytest.raises(ValueError) as excinfo:
            reader.read(str(p))
        assert "JSON должен быть объектом (словарем)" in str(
            excinfo.value)
    ...
    def test_read_json_with_float_scores(self, tmpdir):
        ...
        assert any(subject[0] == "математика" and subject[1] == 85.5
                   for subject in subjects)
        assert any(subject[0] == "физика" and subject[1] == 90.0
                   for subject in subjects)
    ...
    def test_file_encoding_handling(self, tmpdir):
        ...
        assert any("русский язык" in subject for subject in
                   result["Ковальчук Анна Михайловна"])
